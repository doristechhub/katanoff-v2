...
const getActiveProductsByIds = (productIds) => {
  return new Promise(async (resolve, reject) => {
    try {
      const productsByIds = await getProductsByIds(productIds);
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const latestProduct = helperFunctions
        ...
        .slice(0, length)
        .map((product) => {
          const { white, yellow, rose } =
            helperFunctions?.getGoldColorWiseMedia(product);
        ...
        if (productData) {
          ...
          const sortedReletedProductData = helperFunctions
            ...
            .map((foundProduct) => {
              ...
              const { white, yellow, rose } =
                helperFunctions?.getGoldColorWiseMedia(foundProduct);
              return {
                id: foundProduct.id,
    ...
    try {
      ...
      filteredProducts.forEach((element) => {
        ...
        if (productTypeItems?.length) {
          tempProductTypes.push(...productTypeItems?.map((pt) => pt?.title));
        }
    ...
    try {
      ...
      const searchResults = allActiveProductsData.filter((product) => {
        ...
        if (
          ...
          product.subCategoryNames.forEach((subCategory) => {
            fieldsToSearch.push(subCategory?.title);
          });
        }
        ...
        if (
          ...
          product.productTypeNames.forEach((productType) => {
            fieldsToSearch.push(productType?.title);
          });
        }
