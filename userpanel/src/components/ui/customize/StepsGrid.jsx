        ...
        {steps.map((step, index) => (
          <div key={`breadcrumb-${index}`} className="flex items-center gap-4">
            <div className="flex flex-col items-start gap-0.5">
              ...
              <div className="hidden sm:grid grid-cols-[auto_1fr] gap-x-2 text-sm md:text-base lg:text-lg xl:text-xl">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {step?.subOption?.length > 0 && (
                    <div className="hidden sm:flex flex-wrap gap-2 text-xs md:text-sm text-gray-500 pt-1">
              ...
              {/* {step?.subOption?.length > 0 && (
                <div className="hidden sm:flex gap-2 text-xs md:text-sm text-gray-500 mx-auto">
                  {step.subOption.map((sub, subIdx) => (
                    <Link
                      key={subIdx}
                      href={sub.route}
